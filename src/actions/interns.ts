...
export async function createInternAction(formData: FormData) {
  ...
  if (profile.role !== "lead") throw new Error("Only leads can add employees.");

  const parsed = internCreateSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) throw new Error("Enter the employee name and a valid email if provided.");
